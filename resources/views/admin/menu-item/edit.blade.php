                            ...
                            @if(count(json_decode($menu_item->routeparams, true)) > 0)
                                ...
                                @foreach(json_decode($menu_item->routeparams, true) as $param=>$value)
                                    <div class="input-group input-group-sm mb-2" data-id="{{ $i }}">
                                        <input type="text" name="params[]" class="form-control form-control-sm" value="{{ $param }}" placeholder="Parameter">
                                        <input type="text" name="values[]" class="form-control form-control-sm" value="{{ $value }}" placeholder="Value">
                                        <button type="button" class="btn btn-sm btn-outline-secondary btn-add-row" data-id="{{ $i }}"><i class="bi-plus"></i></button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary btn-delete-row" data-id="{{ $i }}"><i class="bi-trash"></i></button>
                                    </div>
                                    @php $i++; @endphp
                                @endforeach
                            @else
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="visible_radio" id="visible-radio-1" value="1" {{ $menu_item->visible_conditions == '' ? 'checked' : '' }}>
                                <label class="form-check-label" for="visible-radio-1">
                                    Selalu Terlihat
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="visible_radio" id="visible-radio-2" value="2" {{ is_int(strpos($menu_item->visible_conditions, 'Auth::user()->role_id')) ? 'checked' : '' }}>
                                <label class="form-check-label" for="visible-radio-2">
                                    Terlihat Berdasarkan Role
                                </label>
                            </div>
                            <div class="card border my-2 {{ is_int(strpos($menu_item->visible_conditions, 'Auth::user()->role_id')) ? '' : 'd-none' }}" id="card-role">
                                <div class="card-body p-2 ps-4">
                                    @foreach($roles as $role)
                                        ...
                                        @else
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="checkbox" name="role" id="role-{{ $role->id }}" value="{{ $role->code }}" {{ is_int(strpos($menu_item->visible_conditions, 'Auth::user()->role_id')) && is_int(strpos($menu_item->visible_conditions, "role('".$role->code."')")) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="role-{{ $role->id }}">{{ $role->name }}</label>
                                            </div>
                                        @endif
                                    @endforeach
                            ...
                            <textarea name="visible_conditions" class="form-control form-control-sm {{ $errors->has('visible_conditions') ? 'border-danger' : '' }} mt-2" rows="3" readonly>{{ $menu_item->visible_conditions }}</textarea>
