...
<script type="text/javascript">
    ...
    Spandiv.DataTable("#datatable", {
		...
		pageLength: 50,
        fixedHeader: true,
        url: Spandiv.URL("{{ route('admin.log.activity.url') }}", {user: "{{ $user }}", range: "{{ $range }}", month: "{{ $month }}", year: "{{ $year }}"}),
        columns: [
            {data: 'url', name: 'url'},
    ...
    $(document).on("change", "select[name=user], select[name=range], select[name=month], select[name=year]", function() {
        ...
        window.location.href = Spandiv.URL("{{ route('admin.log.activity.url') }}", {user: user, range: range, month: month, year: year});
    });
